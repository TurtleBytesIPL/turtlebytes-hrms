...
export class AttendanceService {
  ...
  async checkIn(employeeId: string, dto: CheckInDto) {
    ...
    const existing = await (this.prisma as any).attendance.findUnique({
    ...
    if (existing) {
      return (this.prisma as any).attendance.update({
        ...
        data: { checkIn: now, isLate, lateMinutes, status: AttendanceStatus.PRESENT, approvalStatus: 'PENDING', notes: dto.notes },
      });
    }

    return (this.prisma as any).attendance.create({
      data: { employeeId, date: today, checkIn: now, status: AttendanceStatus.PRESENT, isLate, lateMinutes, approvalStatus: 'PENDING', notes: dto.notes },
    });
  }
  ...
  async findAll(filter: AttendanceFilterDto, user: any) {
    ...
    if (user.role === Role.EMPLOYEE) {
      where.employeeId = user.employeeId;
    } else if ((user.role as string) === 'TEAM_LEAD') {
      const team = await (this.prisma as any).team.findFirst({
        where: { leadId: user.employeeId },
  ...
  async getMonthSummary(employeeId: string, month: number, year: number) {
    ...
    return {
      present:        records.filter((r: any) => r.status === AttendanceStatus.PRESENT).length,
      absent:         records.filter((r: any) => r.status === AttendanceStatus.ABSENT).length,
      late:           records.filter((r: any) => r.isLate).length,
      halfDay:        records.filter((r: any) => r.status === AttendanceStatus.HALF_DAY).length,
      onLeave:        records.filter((r: any) => r.status === AttendanceStatus.ON_LEAVE).length,
      totalWorkHours: records.reduce((acc: number, r: any) => acc + (r.workHours || 0), 0),
      totalOvertime:  records.reduce((acc: number, r: any) => acc + (r.overtime || 0), 0),
      records,
    };
  }
  ...
  async approve(id: string, approverId: string, user: any) {
    ...
    if (user.role !== Role.HR_ADMIN && user.role !== Role.SUPER_ADMIN && (user.role as string) === 'TEAM_LEAD') {
      const team = await (this.prisma as any).team.findFirst({ where: { leadId: user.employeeId } });
      if (!team || record.employee.teamId !== team.id) {
        throw new ForbiddenException('You can only approve your team members');
      }
    ...
    return (this.prisma as any).attendance.update({
      where: { id },
      data: { approvalStatus: 'APPROVED', approvedById: approverId, approvedAt: new Date(), rejectReason: null },
    });
  }
  ...
  async reject(id: string, approverId: string, reason: string, user: any) {
    ...
    if ((user.role as string) === 'TEAM_LEAD') {
      const team = await (this.prisma as any).team.findFirst({ where: { leadId: user.employeeId } });
      if (!team || record.employee.teamId !== team.id) {
        throw new ForbiddenException('You can only reject your team members');
      }
    }
  ...
  async update(id: string, dto: UpdateAttendanceDto, user?: any) {
    ...
    if (user && (user.role as string) === 'TEAM_LEAD') {
      const team = await (this.prisma as any).team.findFirst({ where: { leadId: user.employeeId } });
      if (!team || record.employee.teamId !== team.id) {
        throw new ForbiddenException('You can only edit your team members');
      }
    ...
    return (this.prisma as any).attendance.update({
      ...
      data: {
        ...
        checkIn:  dto.checkIn  ? new Date(dto.checkIn)  : undefined,
        checkOut: dto.checkOut ? new Date(dto.checkOut) : undefined,
        ...(user ? { approvalStatus: 'APPROVED', approvedById: user.employeeId, approvedAt: new Date() } : {}),
      },
    });
  }
  ...
  async getReport(params: {
    ...
    return {
      ...
      summary: {
        ...
        late:    records.filter((r: any) => r.status === 'LATE' || r.isLate).length,

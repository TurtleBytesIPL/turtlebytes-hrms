...
export class RecruitmentController {
  ...
  createCandidate(@CurrentUser() user: any, @Body() dto: CreateCandidateDto) {
    // Use employeeId if available, otherwise fall back to userId (for admins without employee record)
    const creatorId = user?.employeeId || user?.id || null
    return this.recruitmentService.createCandidate(creatorId, dto)
  }

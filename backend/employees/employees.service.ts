  ...
  private async initLeaveBalances(employeeId: string) {
    ...
    await this.prisma.leaveBalance.createMany({
      data: [
        ...
        { employeeId, year, leaveType: LeaveType.SICK, allocated: 10 },
        { employeeId, year, leaveType: LeaveType.CASUAL, allocated: 7 },
      ],
      skipDuplicates: true,
    });
  }
  ...
  async bulkImportFromExcel(rows: any[], defaultDepartmentId: string) {
    ...
    for (const row of rows) {
      try {
        ...
        const fullName = String(row.name || '').trim().replace('.', ' ');
        ...
        if (existing) {
          results.failed.push({ name: fullName, reason: 'Already exists' });
          continue;
        }

        const jobTitle = row.designation || 'Employee';
        ...
        const employee = await this.prisma.employee.create({
          data: {
            ...
            emergencyPhone: row.emergencyContact ? String(row.emergencyContact) : undefined,
            userId: user.id,
          },
        });

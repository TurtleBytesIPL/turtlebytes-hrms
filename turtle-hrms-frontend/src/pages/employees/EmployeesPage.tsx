  ...
  const parseExcel = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const fmtDate = (v: any): string | null => {
        ...
        // Handle "16-Geb-26" typos → try to parse
        try { const d = new Date(s); if (!isNaN(d.getTime())) return d.toISOString().split('T')[0] } catch { }
        return null
      }

      // Helper: auto-generate @turtlebytes.in email from name + empId
      const toTBEmail = (name: string, empId: string): string => {
        const parts = name.trim().replace(/[^a-zA-Z\s.]/g, '').split(/[\s.]+/).filter(Boolean)
        const first = (parts[0] || 'emp').toLowerCase()
        const last = parts.slice(1).join('.').toLowerCase() || empId
        return `${first}.${last}@turtlebytes.in`
      }

      const parsed = dataRows.map((r: any) => {
        const empId = String(r[2] || '').trim()
        const name = String(r[3] || '').trim()
        const excelEmail = r[13] ? String(r[13]).trim() : null
        // Always use @turtlebytes.in — ignore personal gmail from sheet
        const email = toTBEmail(name, empId)
        return {
          empId,
          name,
          email,
          phone: r[12] ? String(r[12]).replace(/[^0-9]/g, '').slice(0, 10) : null,
          joiningDate: fmtDate(r[6]),
          dateOfBirth: fmtDate(r[7]),
          bloodGroup: r[9] ? String(r[9]).trim() : null,
          designation: r[10] ? String(r[10]).trim().replace(/\s+/g, ' ') : 'Employee',
          emergencyContact: r[14] ? String(r[14]).replace(/[^0-9]/g, '').slice(0, 10) : null,
          maritalStatus: r[15] ? String(r[15]).trim() : null,
        }
      }).filter(r => r.empId && r.name) // final safety filter
...
export default function EmployeesPage() {
  ...
  const createMut = useMutation({
    mutationFn: (d: any) => {
      // Clean up empty strings that could fail validation
      const clean: any = {}
      Object.entries(d).forEach(([k, v]) => {
        if (v !== '' && v !== null && v !== undefined) clean[k] = v
      })
      // Convert numeric strings
      if (clean.ctc) clean.ctc = Number(clean.ctc)
      if (clean.basicPercent) clean.basicPercent = Number(clean.basicPercent)
      // Strip ctc/basicPercent from main payload — sent separately if needed
      const { ctc, basicPercent, ...employeePayload } = clean
      if (ctc) employeePayload.ctc = ctc
      if (basicPercent) employeePayload.basicPercent = basicPercent
      return selected ? employeesApi.update(selected.id, employeePayload) : employeesApi.create(employeePayload)
    },
    onSuccess: (res) => {
    ...
    onError: (err: any) => {
      ...
      toast.error(Array.isArray(msg) ? msg[0] : (msg || 'Failed to save employee. Check required fields.'))
    },
  })
    ...
    <div>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="card overflow-hidden">
          <Table headers={isSuperAdmin ? ['Employee', 'Department', 'Designation', 'Monthly Salary', 'Status', 'Joined', 'Actions'] : ['Employee', 'Department', 'Designation', 'Status', 'Joined', 'Actions']} loading={isLoading}>
                ...
                {isSuperAdmin && (
                  <td className="table-td">
                    {emp.salaryStructure?.ctc
                      ? <span className="font-semibold text-emerald-700">₹{(emp.salaryStructure.ctc / 12).toLocaleString('en-IN', { maximumFractionDigits: 0 })}<span className="text-xs text-slate-400 font-normal">/mo</span></span>
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setSelected(null) }}
        ...
        <form onSubmit={handleSubmit((d) => createMut.mutate(d))}>
          <div className="grid grid-cols-2 gap-4">
            ...
            {isHR && (
              <>
                <p className="col-span-2 text-xs font-bold text-slate-400 uppercase tracking-widest border-b pb-2 pt-2">Salary Info (HR Only)</p>
                <FormField label="Annual CTC (₹)">
                  <input className="input" type="number" placeholder="e.g. 600000" {...register('ctc')} />
                </FormField>
                <FormField label="Basic % of CTC">
                  <input className="input" type="number" defaultValue={40} placeholder="40" {...register('basicPercent')} />
                </FormField>
              </>
            )}

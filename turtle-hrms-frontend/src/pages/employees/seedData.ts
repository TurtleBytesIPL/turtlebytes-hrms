...
async function main() {
  ...
  const dataDept = await prisma.department.upsert({
    where: { code: 'DP' },
    update: {},
    create: { name: 'Data Processing', code: 'DP', description: 'Data Processing Department' },
  });
  ...
  const itDept = await prisma.department.upsert({
    where: { code: 'IT' },
    update: {},
    create: { name: 'Information Technology', code: 'IT', description: 'IT & Software Department' },
  });
  ...
  const adminDept = await prisma.department.upsert({
    where: { code: 'ADMIN' },
    update: {},
    create: { name: 'Admin', code: 'ADMIN', description: 'Admin Department' },
  });
  ...
  for (const emp of EXCEL_EMPLOYEES) {
    ...
    // Role: HR jobTitle → HR_ADMIN, else EMPLOYEE
    ...
    console.log(`  ✅ ${emp.empId}  ${(emp.firstName + ' ' + emp.lastName).trim().padEnd(30)}  ${emp.email.padEnd(48)}  ${password}`);
  }
  ...
  console.log('  ✅ Standard salary structure created\n');
  ...
  console.log('');
  console.log('   Example  :  priyanka.p@turtlebytes.in    /  Pass@12240009');

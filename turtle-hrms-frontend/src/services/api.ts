...
//   byDepartment: (departmentId: string) => api.get(`/employees/department/${departmentId}`),
//   uploadPhoto: (employeeId: string, formData: FormData) =>
//     api.post(`/employees/${employeeId}/photo`, formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
//   deletePhoto: (employeeId: string) => api.delete(`/employees/${employeeId}/photo`),
// }
...
//   approve: (id: string) => api.patch(`/attendance/${id}/approve`),
//   reject: (id: string, reason: string) => api.patch(`/attendance/${id}/reject`, { reason }),
...
//   assignMembers: (id: string, employeeIds: string[]) => api.post(`/teams/${id}/assign-members`, { employeeIds }),
//   getAttendance: (id: string, params?: any) => api.get(`/teams/${id}/attendance`, { params }),
...
//     const token = localStorage.getItem('hrms_token')
//     const base = (import.meta as any).env?.VITE_API_URL || 'http://localhost:3000/api/v1'
//     const res = await fetch(`${base}/assets/export`, { headers: { Authorization: `Bearer ${token}` } })
